...
# http://ai.standford.edu/~amaas/data/sentiment/ ## data
train_text, train_labels = read_imdb_split('')
...
# ensure that all of our sequences are padded to the same length and are tuncated to be no longer than model's
...
trainer = Trainer(
    ...
    eval_dataset=val_dataset                   # evaluationd dataset
)
trainer.train()
...
model = DistilBertForSequenceClassification.from_pretrained('distilber-base-uncased')
model.to(device)
model.train()
